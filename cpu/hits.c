...
int main(int ac, char** av) {
	...
	char temp[2048];
	temp[0] = '\0';
	sprintf(temp, "%s-%s-K32.hits.nseqs", av[1], av[2]);
	FILE * nseqs_write = fopen(temp, "wt");
	if(nseqs_write == NULL){ fprintf(stderr,"Could not write number of sequences in Y file %s", temp); exit(-1); }
	...
	while (!feof(fX1) && !feof(fY1)) {
		...
		for (i = 0; i < heX.num; i += stepX)
			for (j = 0; j < heY.num; j += stepY) {
				...
				if(hBuf[hitsInBuf].seqY > n_seqs_y) n_seqs_y = hBuf[hitsInBuf].seqY;
	...
	// Write out number of y-seqs
	// Homo_sapiens.GRCh37.68.dna.chromosome.1.fa-Macaca_mulatta.MMUL_1.68.dna.chromosome.1.fa-K32.hits.nseqs
	
	fprintf(nseqs_write, "%"PRIu64, n_seqs_y);
	fclose(nseqs_write);
