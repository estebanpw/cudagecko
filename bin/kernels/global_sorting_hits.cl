...
__kernel void kernel_sort(__global hitGPU * in, __global parameter_sort * params)
{
        ...
        if(j < N && i < j){
            ...
            if(is_j_smaller_than_i == 1 && dir == 0){
                ...
                */
                ...
                if(in[i].seq_x != in[j].seq_x){ // XOR with its own will produce 0
                    in[i].seq_x = in[i].seq_x ^ in[j].seq_x;
                    in[j].seq_x = in[j].seq_x ^ in[i].seq_x;
                    in[i].seq_x = in[i].seq_x ^ in[j].seq_x;
                }

                if(in[i].seq_y != in[j].seq_y){
                    in[i].seq_y = in[i].seq_y ^ in[j].seq_y;
                    in[j].seq_y = in[j].seq_y ^ in[i].seq_y;
                    in[i].seq_y = in[i].seq_y ^ in[j].seq_y;
                }
